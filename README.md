...
其中，大磊FPGA的源代码包括一些的数字图像处理的模块。我自己的源代码则直接将Vivado 2022.1工程“ov5640_fun4_lcd_up3”(**这个工程在```ov5640_fun4_lcd_up3.txt```里面！！！**)放了上去，同时将工程中的```比特流```、```源代码及硬件约束文件``` 单独拿了出来，需要的同学可以快速查看。
...
本markdown文档中用到的图片。另外，PPT展示中的成果展示视频也放上去了，见```0-成果展示-机位1.mp4```和```0-成果展示-机位2.mp4```。再另外，PPT模板也放到里面了，见```0-模板2.pptx```。

## 2. 程序移植说明

本人在开发时使用到的硬件：
> - FPGA开发工具：Vivado 2022.1。
> - 开发板：正点原子达芬奇PRO(xc7a100tfgg484-2)。
> - 摄像头：正点原子OV5640。
